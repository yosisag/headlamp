...
| [formatDuration](functions/formatDuration.md) | Format a duration in milliseconds using either compact or detailed style. |
...
| [isValidTimezone](functions/isValidTimezone.md) | Returns true when tz is a valid IANA timezone string accepted by the Intl API. Some Linux systems expose TZ=:/etc/localtime which Chrome resolves to "Etc/Unknown" — an identifier that Node accepts but browsers reject with a RangeError. |
| [localeDate](functions/localeDate.md) | - |
| [normalizeUnit](functions/normalizeUnit.md) | - |
| [timeAgo](functions/timeAgo.md) | Returns the time elapsed since the given date. |
